...
class HiAL(BaseVideoDataset):
    """ HiAl dataset for RGB-T tracking.
    """

    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        """
        args:
            root - path to the hial training data.
            image_loader (jpeg4py_loader) -  The function to read the images. jpeg4py (https://github.com/ajkxyz/jpeg4py)
                                w            is used by default.
            split - 'train' or 'test'.
            seq_ids - List containing the ids of the videos to be used for training. Note: Only one of 'split' or 'seq_ids'
                        options can be used at the same time.
            data_fraction - Fraction of dataset to be used. The complete dataset is used by default
        """
        root = env_settings().hial_dir if root is None else root
        super().__init__('HiAl_add', root, image_loader)
