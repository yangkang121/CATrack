...
def get_x_frame(color_path, depth_path, dtype='rgbcolormap', depth_clip=False):
    ...
    if depth_path:
        dp = cv2.imread(depth_path, -1) # depth: (H, W); event/thermal: (H,W,3)
    ...
    elif dtype == 'RGBD' or dtype == 'rgbcolormap':
        dp = cv2.normalize(dp, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        dp = np.asarray(dp, dtype=np.uint8)
        colormap = cv2.applyColorMap(dp, cv2.COLORMAP_JET)  # (h,w) -> (h,w,3)
        # cv2.imwrite('/home/data/yz/2024cvpr/anytrack/debug/1.png', colormap)
    ...
    elif dtype == 'RGBT' or dtype == 'RGBE' or dtype == 'rgbrgb':
        dp = cv2.cvtColor(dp, cv2.COLOR_BGR2RGB)
        # cv2.imwrite('/home/data/yz/2024cvpr/anytrack/debug/thermal.png', dp)
        img = cv2.merge((rgb, dp))  # (H, W, 6)
