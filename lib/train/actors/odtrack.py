...
class ODTrackActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        # compute losses
        loss, status = self.compute_losses(out_dict, data['visible']) # loss, status = self.compute_losses(out_dict, data)
    ...
    def forward_pass(self, data):
        ...
        for i in range(self.settings.num_template):
            template_img_v = data['visible']['template_images'][i].view(-1, *data['visible']['template_images'].shape[2:])  # (batch, 3, 128, 128)
            template_img_i = data['infrared']['template_images'][i].view(-1, *data['infrared']['template_images'].shape[2:])  # (batch, 3, 128, 128)
            template_list.append([template_img_v,template_img_i])
        ...
        for i in range(self.settings.num_search):
            search_img_vt = data['visible']['search_images'][i].view(-1, *data['visible']['search_images'].shape[2:])  # (batch, 3, 320, 320)
            search_img_it = data['infrared']['search_images'][i].view(-1, *data['infrared']['search_images'].shape[2:])  # (batch, 3, 320, 320)
            search_list.append([search_img_vt, search_img_it])
        ...
        if self.cfg.MODEL.BACKBONE.CE_LOC:
            for i in range(self.settings.num_template):
                box_mask_z.append(generate_mask_cond(self.cfg, template_list[i][0].shape[0], template_list[i][0].device,
                                                    data['visible']['template_anno'][i]))
