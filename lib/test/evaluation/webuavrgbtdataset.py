...
class WebUAVRGBTDataset(BaseDataset):
    """ WebUAVRGBT dataset for RGB-T tracking.
    """
    def __init__(self, split):
        super().__init__()
        # Split can be test, val, or ltrval (a validation split consisting of videos from the official train set)
        if split == 'testingset' or split == 'val':
            self.base_path = os.path.join(self.env_settings.webuavrgbt_path, split)
        else:
            self.base_path = os.path.join(self.env_settings.webuavrgbt_path)
    ...
    def _construct_sequence(self, sequence_name):
        ...
        frames_path_i = '{}/{}/img'.format(self.base_path, sequence_name)
        frames_path_v = '{}/{}/img'.format(self.base_path, sequence_name)
        ...
        return Sequence(sequence_name, frames_list, 'WebUAVRGBT', ground_truth_rect.reshape(-1, 4))
    
    def __len__(self):
        return len(self.sequence_list)

    def _get_sequence_list(self, split):
        with open('{}/testingsetList.txt'.format(self.base_path)) as f:  # 需要生成一个包含所有测试序列名字的txt，/public/workspace/xyl/ODTrack-RGBT/dir2txt-xyl.py
