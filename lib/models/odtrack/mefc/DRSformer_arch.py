...
# from plotting_xyl import show_tensor
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        # ---------------------------------------------------------------------------------------- xyl 3维输入转换成4维
        flag_3to4 = False
        if len(x.size()) == 3:  # bs, seq_len 序列长度，embed_dim 通道数：32*256*192
        ...
        attn = (q @ k.transpose(-2, -1)) * self.temperature  # C*HW x HW*C，标准自注意力是 HW*C x C*HW
...
class OperationLayer(nn.Module):
    ...
    def forward(self, x, weights):
        weights = weights.transpose(1, 0)  # w:(8,1)
        states = []
        for w, op in zip(weights, self._ops):
            states.append(op(x) * w.view([-1, 1, 1, 1]))  # w:(1,1,1,1)
        return self._out(torch.cat(states[:], dim=1))  # 多个专家的堆叠，然后self._out变回原来的输入维度
...
class GroupOLs(nn.Module):
    ...
    def forward(self, s0, weights):
        ...
        for i in range(self._steps):
            res = s0
            s0 = self._ops[i](s0, weights[:, i, :])  # steps个专家网络OperationLayer级联处理的
            s0 = self.relu(s0 + res)
        return s0
...
class subnet(nn.Module):
    def __init__(self, dim, layer_num=1, steps=4):
        ...
        for _ in range(self._layer_num):  # layer_num 是OALayer+GroupOLs的重复次数
            attention = OALayer(self._C, self._steps, self.num_ops)  # 专家数量为：steps * num_ops
            self.layers += [attention]
            layer = GroupOLs(steps, self._C)  # steps 是OperationLayer（专家个数为num_ops）的重复次数
            self.layers += [layer]

    def forward(self, x):

        for _, layer in enumerate(self.layers):
            if isinstance(layer, OALayer):  # OAlayer 得到权重
                weights = layer(x)
                weights = F.softmax(weights, dim=-1)
            else:
                x = layer(x, weights)  # GroupOLs 根据权重对专家加权
...
if __name__ == '__main__':
    ...
    if TKSA:
        # ===================================================================== TKSA：Top-k选择注意力
        print('=================== TKSA：Top-k选择注意力 ===================')
        # input = torch.rand(1, 3, 256, 256)
        # input = torch.rand(32, 256, 192)
        ...
        # model = DRSformer()
        model = Attention(dim=3, num_heads=1, bias=False)
        # model = Attention(dim=192, num_heads=1, bias=False)
        output = model(input)

        path = os.path.join(os.path.abspath(os.getcwd()) + '/../../../vis')
        print('可视化的保存路径：', path)
        show_tensor(input.sum(dim=1), fig_num=0, title='input', path=path)
        show_tensor(output.sum(dim=1), fig_num=0, title='output', path=path)
