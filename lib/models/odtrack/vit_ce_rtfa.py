...
from .hgit.bra_legacy import BiLevelRoutingAttention
...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # self.hgit = HGIT(nn.BatchNorm2d, dim=embed_dim, num_clusters=8, dropout=0.1)
        self.rtfa = BiLevelRoutingAttention(dim=embed_dim, num_heads=8, n_win=8, qk_dim=embed_dim,
                                kv_per_win=-1, kv_downsample_ratio=4,
                                kv_downsample_kernel=4, kv_downsample_mode='identity',
                                topk=1, param_attention='qkvo', param_routing=False,
        ...
        for i, blk in enumerate(self.blocks):
            if self.add_cls_token:  # cfg.MODEL.BACKBONE.ADD_CLS_TOKEN = True   # x_v: torch.Size([1, 321, 768])
                ...
                if tbsi_loc is not None and i in tbsi_loc:
                    # x_v_ori = x_v
                    # recover x_v to go through rtfa blocks
                    lens_z_v_new = lens_z
                    lens_x_v_new = lens_x
                    ...
                    # z_v_feat = token2feature(z_v_tokens)  # torch.Size([1, 768, 8, 8])  # 不需要模板进行交互
                    x_v_feat = token2feature(x_v_tokens)  # torch.Size([1, 768, 16, 16])
                    x_v_rtfa_feat = self.rtfa(x_v_feat.permute(0, 2, 3, 1))  # torch.Size([1, 16, 16, 768])

                    # x_i_ori = x_i
                    # recover x_i to go through rtfa blocks
                    ...
                    x_i_rtfa_feat = self.rtfa(x_i_feat.permute(0, 2, 3, 1))  # torch.Size([1, 16, 16, 768])
                    
                    x_v_rtfa_token = feature2token(x_v_rtfa_feat.permute(0, 3, 1, 2))  # torch.Size([1, 256, 768])
                    x_v = combine_tokens(z_v_tokens, x_v_rtfa_token, mode=self.cat_mode)  # torch.Size([1, 320, 768])

                    x_i_rtfa_token = feature2token(x_i_rtfa_feat.permute(0, 3, 1, 2))  # torch.Size([1, 256, 768])
                    x_i = combine_tokens(z_i_tokens, x_i_rtfa_token, mode=self.cat_mode)  # torch.Size([1, 448, 768])
