...
from .improve.CMGF import FusionModule
from .improve.AAAI2026_DPFFEM import FrePromptMoE
from .improve.AAAI2026_MECM import Memory_Expert_Compensation_Module
...
class ODTrack(nn.Module):
    ...
    def __init__(self, transformer, box_head, aux_loss=False, head_type="CORNER", token_len=1):
        ...
        # self.fusion = FusionModule(in_channels=hidden_dim)
        # self.expert = Memory_Expert_Compensation_Module(channels=hidden_dim * 2, num_experts=8, top_k=2, memory_size=64)
        # 消融实验:8(完成) 2(完成) 4(完成)6(完成)10(完成)12(完成)18(完成)
        # 256*256
        self.expert = FrePromptMoE(img_size=16, input_size=hidden_dim, output_size=hidden_dim, num_experts=8, hidden_size=64, noisy_gating=True, k=2, trainingmode=True)
        # 384*384
        # self.expert = FrePromptMoE(img_size=24, input_size=hidden_dim, output_size=hidden_dim, num_experts=8, hidden_size=64, noisy_gating=True, k=2, trainingmode=True)
    ...
    def forward_head(self, cat_feature, frame, gt_score_map=None):
        ...
        # ================================================yk 特征融合=================================================
        if 1:
            enc_opt1_feat = token2feature(enc_opt1)  # torch.Size([1, 768, 16, 16]) RGB
            enc_opt2_feat = token2feature(enc_opt2)  # torch.Size([1, 768, 16, 16]) TIR
            # enc_opt1_feat_hgit, enc_opt2_feat_hgit = self.hgit(enc_opt1_feat, enc_opt2_feat)  # torch.Size([1, 768, 16, 16])
            
            enc_opt1_feat_rgb, _ = self.expert(enc_opt1_feat,enc_opt2_feat)
            enc_opt1_feat_tir, _ = self.expert(enc_opt2_feat,enc_opt1_feat)

            enc_opt1_token_hgit = feature2token(enc_opt1_feat_rgb)  # torch.Size([1, 256, 768])
            enc_opt2_token_hgit = feature2token(enc_opt1_feat_tir)  # torch.Size([1, 256, 768])
        ...
        # =======================================yk 模态加权==========================================================
        if 0: # 1:使用专家模块，0：不使用专家模块
            opt_feat_mefc = self.expert(opt_feat.permute((0, 3, 1, 2)), train = True)
            # print(f"专家模块已使用：{self.expert}")  # [1, 16, 16, 1536] > [1, 1536, 16, 16] > torch.Size([1, 1536, 16, 16])
            opt_feat = self.rgbt_fuse_search(opt_feat_mefc[0])  # 只取第一个元素（实际特征），其余为losses和cof
        else:
            opt_feat = self.rgbt_fuse_search(opt_feat.permute((0, 3, 1, 2)))
        # =======================================yk 模态加权=========================================================
        # bs, Nq, C, HW = opt.size()
...
def build_odtrack(cfg, training=True):
    ...
    # print(f"current_dir:{current_dir}!!!!!!!!!!!!!!!!!!!!!!!!!11")
    # pretrained_path = os.path.join(current_dir, '../../../pretrained_networks')
    # pretrained_path = os.path.abspath(pretrained_path)
    pretrained_path = "/media/root123/date-5T/YK-Track_code/FMTrack/pretrained_networks"
    if cfg.MODEL.PRETRAIN_FILE and ('ODTrack' not in cfg.MODEL.PRETRAIN_FILE) and training: # ('OSTrack' not in cfg.MODEL.PRETRAIN_FILE) and training:
        ...
        print('Firstly,Load pretrained model from: ' + pretrained)
    else:
        pretrained = ''
    ...
    if 'ODTrack' in cfg.MODEL.PRETRAIN_FILE and training:
        ...
        # print(f"pretrained_file = os.path.join(pretrained_path, cfg.MODEL.PRETRAIN_FILE):{pretrained_file}") # 查看路径
        checkpoint = torch.load(pretrained_file, map_location="cpu")
        missing_keys, unexpected_keys = model.load_state_dict(checkpoint["net"], strict=False)
        print('Secondly, Load pretrained model from: ' + cfg.MODEL.PRETRAIN_FILE)

        # 如果checkpoint是一个字典，可能包含了模型权重、优化器状态等
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            # model.load_state_dict(checkpoint['net']) # yk修改，不改显示缺少rgbt融合参数
        ...
        # torch.save(model.state_dict(), '/home/a806/newdriver/YK-Track_code/FMTrack/output/ODTrack_ep0014.pth.tar')
        # print(f"模型权重已保存到 {'/home/a806/newdriver/YK-Track_code/FMTrack/output'}")
    return model
