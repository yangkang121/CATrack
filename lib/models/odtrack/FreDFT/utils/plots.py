...
from .general import xywh2xyxy, xyxy2xywh, xywhn2xyxy
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            # 文件的命名格式 层名+层的索引
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features_C3_Single.png"  # filename
            # 按通道数拆分Tensor
            # 进行逐通道的可视化
...
def feature_visualization_all(x, module_type, stage,  save_dir=Path('runs/detect/exp')):
    ...
    if 'Detect' not in module_type:
        batch, channels, height, width = x.shape  # batch, channels, height, width
        if height > 1 and width > 1:
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features_FDFTM.jpg"  # filename
    ...
    if 'Detect' not in module_type:
        ...
        y0 = x[0]
        if height > 1 and width > 1:
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features_RGB_FDFTM.jpg"  # filename
            img = y0[0].cpu().transpose(0, 1).sum(1).detach().numpy()
            plt.imsave(f, img)
            logger.info(f'Saving {f}...')
        ...
        y1 = x[1]
        if height > 1 and width > 1:
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features_IR_FDFTM.jpeg"  # filename
            img = y1[0].cpu().transpose(0, 1).sum(1).detach().numpy()
            plt.imsave(f, img)
            logger.info(f'Saving {f}...')
    """
