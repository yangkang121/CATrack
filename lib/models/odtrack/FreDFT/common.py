...
class Fusion(nn.Module):
    def __init__(self,dim):
        ...
        self.LFEM = LFEM(dim)
        self.cgmm = NLFEM(dim)
        self.fdftm = FDFTM(dim)

    def forward(self, x):
        x_lfem = self.LFEM(x)
        x_cgmm = self.cgmm(x_lfem)
        rgb_fea, ir_fea = self.fdftm(x_cgmm)

        return rgb_fea, ir_fea

class Fusion_LFEM_CGMM(nn.Module):
    def __init__(self,dim):
        super(Fusion_LFEM_CGMM, self).__init__()
    ...
    def forward(self, x):
        x_lfem = self.LFEM(x)
        x_cgmm = self.cgmm(x_lfem)
        rgb_fea, ir_fea = x_cgmm[0], x_cgmm[1]
