...
class TopkRouting(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor) -> Tuple[Tensor]:
        ...
        # ----------------------------------------------------------------------------------------------------- 局部密度+距离指示器，做聚类，公式4，改进了Biformer
        # DPC-KNN
        ...
        density = (-(topk_attn_logit ** 2).mean(dim=-1)).exp()  # Step 2 计算局部密度
        density = density + torch.rand(
            density.shape, device=density.device, dtype=density.dtype) * 1e-6
        mask = density[:, None, :] > density[:, :, None]
        mask = mask.type(query.dtype)
        dist_max = attn_logit.flatten(1).max(dim=-1)[0][:, None, None]
        dist, index_parent = (attn_logit * mask + dist_max * (1 - mask)).min(dim=-1)  # 计算距离指示器
        score = dist * density  # 计算分数。分数越高，表明该token对伪装对象的识别提供了信息丰富的视觉语义
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        r_weight, r_idx = self.router(q_win,
                                      k_win)  # both are (n, p^2, topk) tensors  # 局部密度+距离指示器，做聚类，公式4，对biformer进行了改进
...
if __name__ == '__main__':
    ...
    input = torch.randn(1, 32, 32, 96).to(device)  # 针对的是 transformer 的输入：nhwc，之前我的Biformer针对的输入是 nchw
    ...
    m = BiLevelRoutingAttention(dim=96, num_heads=8, n_win=8, qk_dim=96,
