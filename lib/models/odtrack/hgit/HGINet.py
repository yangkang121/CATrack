...
class HGIT(nn.Module): 
    ...
    def forward(self, feat1, feat2):
        ...
        graph_2, assign_2 = self.proj_2(feat2)  # 潜在空间图映射，得到图的节点
        graph_1, assign_1 = self.proj_1(feat1)  # 潜在空间图映射，得到图的节点
        ...
        mutual_V = torch.cat((graph_2, graph_1), dim=1)  # 层级图交互
        mutual_V_new = self.conv3(mutual_V.unsqueeze(3)).squeeze(3) # V

        enhanced_feat1 = self.gt1(q_graph1, k_graph2, mutual_V_new, assign_1)  # Transformer学习和反映射
...
'''
add token transfer to feature  # copy from ViPT
'''
...
if __name__ == '__main__':
    ...
    input = torch.randn(1, 256, 31, 31).to(device)
    input1 = torch.randn(1, 256, 64, 64).to(device)
    ...
    m = HGIT(nn.BatchNorm2d, dim=256, num_clusters=8, dropout=0.1).to(device)

    output1, output2 = m(input, input1)  # tensor to list
    print('input:{} \ninput1:{}'.format(input.shape, input1.shape))
    print('output1:{} \noutput2:{}'.format(output1.shape, output2.shape))
