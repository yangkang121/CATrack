...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.mefc = subnet(dim=embed_dim)
        ...
        for i, blk in enumerate(self.blocks):
            if self.add_cls_token:  # cfg.MODEL.BACKBONE.ADD_CLS_TOKEN = True   # x_v: torch.Size([1, 321, 768])
                ...
                if tbsi_loc is not None and i in tbsi_loc:
                    # x_v_ori = x_v
                    # recover x_v to go through mefc blocks
                    lens_z_v_new = lens_z
                    lens_x_v_new = lens_x
                    ...
                    x_v_mefc_feat = self.mefc(x_v_feat)  # torch.Size([1, 768, 16, 16])

                    # x_i_ori = x_i
                    # recover x_i to go through mefc blocks
                    ...
                    x_i_mefc_feat = self.mefc(x_i_feat)  # torch.Size([1, 768, 16, 16])
                    
                    x_v_mefc_token = feature2token(x_v_mefc_feat)  # torch.Size([1, 256, 768])
                    x_v = combine_tokens(z_v_tokens, x_v_mefc_token, mode=self.cat_mode)  # torch.Size([1, 320, 768])

                    x_i_mefc_token = feature2token(x_i_mefc_feat)  # torch.Size([1, 256, 768])
                    x_i = combine_tokens(z_i_tokens, x_i_mefc_token, mode=self.cat_mode)  # torch.Size([1, 448, 768])
