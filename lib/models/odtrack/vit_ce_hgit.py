        ...
        for i, blk in enumerate(self.blocks):
            if self.add_cls_token:  # cfg.MODEL.BACKBONE.ADD_CLS_TOKEN = True   # x_v: torch.Size([1, 321, 768])
                ...
                if tbsi_loc is not None and i in tbsi_loc:
                    ...
                    # z_i_feat = token2feature(z_i_tokens)  # torch.Size([1, 768, 8, 8])
                    x_i_feat = token2feature(x_i_tokens)  # torch.Size([1, 768, 16, 16])

                    # z_v_hgit, z_i_hgit = self.hgit(z_v_feat, z_i_feat)  # torch.Size([1, 768, 8, 8])  # HGIT的输入可以是不同大小的
                    x_v_hgit, x_i_hgit = self.hgit(x_v_feat, x_i_feat)  # torch.Size([1, 768, 16, 16])

                    # z_v_hgit_token = feature2token(z_v_hgit)  # torch.Size([1, 192, 768])
                    x_v_hgit_token = feature2token(x_v_hgit)  # torch.Size([1, 256, 768])
                    # x_v = combine_tokens(z_v_hgit_token, x_v_hgit_token, mode=self.cat_mode)  # torch.Size([1, 320, 768])
                    x_v = combine_tokens(z_v_tokens, x_v_hgit_token, mode=self.cat_mode)  # torch.Size([1, 320, 768])

                    # z_i_hgit_token = feature2token(z_i_hgit)  # torch.Size([1, 192, 768])
                    x_i_hgit_token = feature2token(x_i_hgit)  # torch.Size([1, 256, 768])
                    # x_i = combine_tokens(z_i_hgit_token, x_i_hgit_token, mode=self.cat_mode)  # torch.Size([1, 448, 768])
