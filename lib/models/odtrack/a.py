...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # Patch Embedding 将图像分割为固定大小的 patch 并线性投影
        ...
        blocks = []
        ce_index = 0
        self.ce_loc = ce_loc # ce_loc=[3,6,9]
        # CE Block 构建
        ...
        self.FreqFusion = DWT_WindowAttention_SW(dim=embed_dim, input_resolution=img_size, num_heads=8, window_size=8, shift_size=4, bias=False)
        # FreqFusion模块用于双模态特征交互
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        # 搜索区域 x: [可见光, 红外]
        x_v = self.patch_embed(x[0]) # 可见光 patch embedding
        x_i = self.patch_embed(x[1]) # 红外 patch embedding
        ...
        # 可见光和红外共享位置编码
        z_v += self.pos_embed_z # 模板位置编码
        x_v += self.pos_embed_x # 搜索区域位置编码
        ...
        for i, blk in enumerate(self.blocks):
            if self.add_cls_token:  # cfg.MODEL.BACKBONE.ADD_CLS_TOKEN = True   # x_v: torch.Size([1, 321, 768])
                # x_v / x_i	更新后的特征序列 [B, N, C]
                # global_index_t_*	Template 区域的全局索引（跟踪模板）
                # global_index_s_*	Search 区域的全局索引（搜索区域）
                # removed_index_s_*	被丢弃的 search token 索引
                # attn_*	注意力权重矩阵
                # 可见光模态处理 每个Transformer Block 返回
                ...
                # 红外光模态处理 每个Transformer Block 返回
                ...
                if tbsi_loc is not None and i in tbsi_loc:
                    ...
                    lens_z_v_new = lens_z # 模板分支长度（192）
                    lens_x_v_new = lens_x # 搜索分支长度（256）
                    ...
                    x_v_hgit = self.FreqFusion(x_v_feat)
                    x_i_hgit = self.FreqFusion(x_i_feat)
