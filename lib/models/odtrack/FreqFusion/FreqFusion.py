...
class FreqFusion(nn.Module):
    def __init__(self,
        ...
        self.content_encoder = nn.Conv2d(  # ALPF generator  --------------------------------- 自适应低通滤波器(AHPF)生成器
        ...
        if self.feature_resample:  # ------------------------------------------------------------------------- 偏移生成器
        ...
        if self.use_high_pass:
            self.content_encoder2 = nn.Conv2d(  # AHPF generator ----------------------------- 自适应高通滤波器(AHPF)生成器
                self.compressed_channels,
...
if __name__ == '__main__':
    ...
    dim = 768
    # input = torch.randn(1, 256, 64, 64).to(device)  # 必须是另外一个输入的2倍
    # input1 = torch.randn(1, 256, 32, 32).to(device)
    input = torch.randn(1, dim, 16, 16).to(device)  # 必须是另外一个输入的2倍
    input1 = torch.randn(1, dim, 16, 16).to(device)
    ...
    m = FreqFusion(hr_channels=dim, lr_channels=dim,
                   ...
                   compressed_channels=(dim + dim) // 8,
                   use_high_pass=True, use_low_pass=True, semi_conv=False, feature_resample_norm=True).to(device)  # semi_conv=True

    output = m(input, input1)  # tensor to list
    print('input:{} \ninput1:{}'.format(input.shape, input1.shape))
    print('===================================================')
    print('output1:{} \noutput2:{} \noutput3:{}'.format(output[0].shape, output[1].shape, output[2].shape))
